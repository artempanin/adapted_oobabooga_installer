...
CMD_FLAGS = '--deepspeed --chat --model-menu --auto-devices'
...
def install_dependencies():
    run_cmd("conda install -y -k mamba", assert_success=True,
            environment=True)
    ...
    if gpuchoice == "a":
        ...
        run_cmd("mamba install -y -k pytorch torchvision torchaudio cpuonly git -c pytorch", assert_success=True, environment=True)
    else:
...
def update_dependencies():
    ...
    if sys.platform.startswith("linux"):
        ...
        if gxx_output.returncode != 0 or b"g++ (GCC) 12" in gxx_output.stdout:
            ...
            run_cmd("mamba install -y -k gxx_linux-64=11.2.0", environment=True)
...
def run_model():
    os.chdir("text-generation-webui")
    run_cmd(f"deepspeed --num_gpus=1 server.py {CMD_FLAGS}", environment=True)
